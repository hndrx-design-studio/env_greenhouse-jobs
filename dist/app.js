...
function applyFilters() {
    ...
    if (selectedSection !== "all") allDepartments.forEach((dept)=>{
        if (dept.id !== selectedSection) dept.style.display = "none";
    });
    // Filter by location
    allJobs.forEach((job)=>{
        if (job.getElementsByClassName("job-location")[0].innerText !== selectedLocation && selectedLocation !== "all") job.style.display = "none";
    });
    // Filter by date
    ...
    allJobs.forEach((job)=>{
        ...
        let daysDiff = timeDiff / 86400000;
        console.log(`daysDiff: ${daysDiff}`);
        console.log(`dateFilter: ${dateFilter.value}`);
        switch(dateFilter.value){
            case "last-week":
                if (daysDiff > 7) job.style.display = "none";
                break;
            case "past-24-hours":
                if (daysDiff > 1) job.style.display = "none";
                break;
            default:
                break;
        }
    ...
    allDepartments.forEach((department)=>{
        ...
        if (visibleJobsInDepartment.length === 0) department.style.display = "none";
    });
...
window.addEventListener("DOMContentLoaded", (event)=>{
    ...
    fetch("https://boards-api.greenhouse.io/v1/boards/" + ghSlug + "/departments/").then(handleError).then((data)=>{
...
function writeJobs() {
    departmentIds.forEach((departmentId)=>{
        ...
        fetch("https://boards-api.greenhouse.io/v1/boards/" + ghSlug + "/departments/" + departmentId).then(handleError).then((data)=>{
...
},{}]},["2ly9l","igcvL"], "igcvL", "parcelRequire78ef")
